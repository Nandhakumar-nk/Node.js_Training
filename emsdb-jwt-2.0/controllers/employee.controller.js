...
const employeeController = {
    ...
    login: async function(req, res, next) {
        ...
        try {
            const employee = await Employee.findOne({ emailID: req.body.emailId }).session(req.addedSession);
    ...
    changePassword: async function(req, res, next) {
        ...
        try {
            ...
            if (employee) {
                req.body.password = await bcrypt.hash(req.body.password, 8);
                await Employee.updateOne({ emailID: req.body.emailId }, { $set: { "password": req.body.password } }, { session: req.addedSession });
                await next();
                message = "password successfully updated for the emailID:" + req.body.emailId;
            } else {
